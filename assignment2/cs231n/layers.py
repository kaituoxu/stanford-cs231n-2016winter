...
def affine_forward(x, w, b):
  ...
  #############################################################################
  mat_x = np.reshape(x, (x.shape[0], -1)) # x in matrix format
  out = np.dot(mat_x, w) + b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b)
...
def affine_backward(dout, cache):
  ...
  mat_x = np.reshape(x, (x.shape[0], -1))
  dw = np.dot(mat_x.T, dout)
...
def relu_forward(x):
  ...
  #############################################################################
  out = np.maximum(x, 0) # element-wise
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = x
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = (dx_bn - np.mean(dx_bn, axis=0) - x_bn*np.mean(dx_bn*x_bn, axis=0)) / std
  dgamma = np.sum(dout * x_bn, axis=0)
  dbeta = np.sum(dout, axis=0)
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def conv_forward_naive(x, w, b, conv_param):
  ...
  Ho = 1 + (H + 2 * pad - HH) / stride # Ho = H' = H out
  Wo = 1 + (W + 2 * pad - WW) / stride
  out = np.zeros((N, F, Ho, Wo))
  # step 2.1: for each filter (for f)
  # step 2.2: for each out height (for ho)
  # step 2.3: for each out width (for wo) 
  for f in xrange(F):
    for ho in xrange(Ho):
      for wo in xrange(Wo):
        ho_beg, wo_beg = ho * stride, wo * stride 
        ho_end, wo_end = ho_beg + HH, wo_beg + WW
        xpad_part = xpad[:, :, ho_beg:ho_end, wo_beg:wo_end] # N x C x HH x WW
        out[:, f, ho, wo] = np.sum(xpad_part * w[f], axis=(1,2,3)) + b[f] # N 
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
...
def conv_backward_naive(dout, cache):
  ...
  xpad = np.pad(x, ((0, 0), (0, 0), (pad, pad), (pad, pad)),
                'constant', constant_values=0)
  # step 2: backward convolution
  N, C, H, W = x.shape
  F, C, HH, WW = w.shape
  stride = conv_param['stride']
  Ho = 1 + (H + 2 * pad - HH) / stride # Ho = H' = H out
  Wo = 1 + (W + 2 * pad - WW) / stride
  # dout -> N x F x Ho X Wo
...
def max_pool_forward_naive(x, pool_param):
  ...
  for ho in xrange(Ho):
    for wo in xrange(Wo):
      ho_beg, wo_beg = ho * stride, wo * stride 
      ho_end, wo_end = ho_beg + ph, wo_beg + pw
      out[:, :, ho, wo] = np.max(x[:, :, ho_beg:ho_end, wo_beg:wo_end], axis=(2,3))
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  Ho = (H - ph) / stride + 1
  Wo = (W - pw) / stride + 1
  dx = np.zeros_like(x) # N x C x H x W
  mask = np.zeros_like(x) # N x C x H x W
  for ho in xrange(Ho):
    for wo in xrange(Wo):
      ho_beg, wo_beg = ho * stride, wo * stride 
      ho_end, wo_end = ho_beg + ph, wo_beg + pw
